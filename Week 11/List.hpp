...
/// @note please spare me if you see a lot of comments, its my psedocode (v.v)
#include <iostream>
#include <ostream>
#include <algorithm>
...
List<T>::List(const List<T>& other)
{
    ...
    if (other.size() > 0) { copyList(other); count = other.size(); }
}
...
{
    for (auto& item : ilist) { insert(end(), item); }
}
...
T& List<T>::front()
{
    if (head == nullptr) { throw std::logic_error("HELP ME_front"); }
    return head->data;
}
...
// T& List<T>::front()
// {
//     return !empty() ? head->data : throw std::logic_error("HELP ME_front");
// }
...
/// EFFECT: remove the first element
// template <class T>
// void List<T>::pop_front()
// { erase(begin()); }

/// remove the last element
/// EFFECT: remove the last element
// template <class T>
...
T& List<T>::back() 
{
    if (tail == nullptr) { throw std::logic_error("HELP ME_back"); }
    return tail->data;
}

/// @paragraph second-appraoch
// template <class T>
// T& List<T>::back() 
// {
...
typename::List<T>::iterator List<T>::erase(List<T>::iterator pos)
{
    ...
    else
    {
        ...
        else if (pos == tail)          { tail = pos->prev; }
    }

    // delete the node;
    count--;

    auto temp = pos.operator->();
...
void List<T>::copyList(const List<T>& other)
{
    ...
    // List<T> *newNode;  // pointer to create a node
    // List<T> *current;  // pointer to traverse the list
    ...
    else
    {
        ...
        {   
            ...
            tail       = newNode;        // make last point to the actual last node
            current    = current->next; 
        }  // end while
    }  // end else
}
